...
const GenerateImages = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate an image of ${input} in a ${styleSelected}`;
        ...
        <div className="flex flex-wrap gap-3 mt-3 sm:max-w-9/11">
          {styleOptions.map((style) => (
            <span
              ...
              className={`px-4 py-1 text-xs rounded-full border cursor-pointer ${styleSelected === style ? "bg-green-50 text-green-700" : "border-gray-300 text-gray-500"}`}
            >
              {style}
            </span>
          ))}
...
{!content?(
  ...
  <div className="h-full mt-3">
    <img src={content} alt={`Image generated about ${input} in ${styleSelected}`} className="w-full h-full"/>
  </div>
)}
