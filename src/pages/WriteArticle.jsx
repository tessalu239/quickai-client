  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        ...
        { prompt, length: selectedLength.text },
        {
        ...
        <div className="flex flex-wrap gap-3 mt-3 sm:max-w-9/11">
          {articleLengthOptions.map((option, index) => (
            <span
              ...
              className={`px-4 py-1 text-xs rounded-full border cursor-pointer ${selectedLength.display === option.display ? "bg-blue-50 text-blue-700" : "text-gray-500 border-gray-300"}`}
              key={index}
            >
              {option.display}
            </span>
